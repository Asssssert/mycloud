<template>
  <el-row>
    <el-card>
      <div slot="header" class="header-title">权限列表</div>
      <div>
        <el-row></el-row>
        <el-row>
          <el-table
  ...
  export default {
    ...
    methods: {
      ...
      batchDelete() {
        ...
        _g.meassageBox('warning', '警告', "确定删除这些类型吗？（删除后无法恢复）")
          .then(() => {
      ...
      handleChild(index, row) {
        ...
        _g.toPageByName("permissionChildManage");
      },
      handleDelete(index, row) {
        _g.meassageBox('warning', '警告', "确定删除该类型吗？（删除后无法恢复）")
          .then(() => {
